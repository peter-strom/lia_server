...
int main(int argc, char *argv[])
{
    ...
    int enable = 1;
    if(setsockopt(serverSd, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(enable)) < 0)
    {
        std::cerr << "Error setsockopt(SO_REUSADDR)" << std::endl;
    }
    ...
    if(bindStatus < 0)
    {
        std::cerr << "Error binding socket to local address :" << errno << std::endl;
        exit(0);
    }
